...
namespace WebApp.Composite.Controllers
{
    ...
    public class CategoryMenuController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            //Cookiden çekebiliriz

            var userId = User.Claims.First(x => x.Type == ClaimTypes.NameIdentifier).Value;

            var categories = await _context.Categories.Include(x => x.Books)
                .Where(x => x.UserId == userId)
                .OrderBy(x => x.Id).ToListAsync();

            //Buradaki Category veritabanı Category için kök, BookComposite ise composite için kök
        ...
        public BookComposite GetMenus(List<Category> categories,Category topCategory,
            BookComposite topBookComposite,BookComposite last=null)
        {
            //Her bir categoriden bir bookComposite her bir Book tanda bir Component oluşturacağaz
